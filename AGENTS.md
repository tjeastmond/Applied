...
- Cmd+K (Mac) / Ctrl+K (Windows) opens the new application modal; hidden lowercase `a` opens Admin when no input focused (no settings-button label); `KeyboardShortcutsHelp` fixed bottom-right lists all shortcuts in a card-styled panel (dismiss click-away, X, Escape) — bubble uses Lucide `KeyboardIcon` in a Next.js-style circle (gray border, `#010101` hover fill/border)
...
- Keyboard shortcut helpers in `src/lib/keyboardShortcut.ts`: `appKeyboardShortcuts()` catalog for the help panel; `isAdminOpenShortcut` (lowercase `a`) wired in `AdminDialog`; shared guards (`isEditableKeyboardTarget`, `consumeDoubleEscape`, mod-key labels) used across `AuthenticatedApp` and detail sheet
