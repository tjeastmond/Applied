...
- Cmd+K (Mac) / Ctrl+K (Windows) opens the new application modal; hidden lowercase `a` opens Admin when no input focused (no settings-button label); when Admin modal is open, lowercase `e` bulk-archives rejected/passed applications without confirmation (Admin section in shortcuts help; Archive button click still shows confirm); `?` toggles `KeyboardShortcutsHelp` (fixed bottom-right; card-styled panel with bold header, rounded corners, card background; Global, Admin, and Detail Drawer sections; dismiss click-away, X, Escape; lowercase key labels with wider tracking); bubble uses Lucide `KeyboardIcon` in a circle (`#333333` border; hover fill/border `#333333`; blurs on click); fades out while detail drawer is open and pop-in animates back on close; **↑/↓** navigate visible application cards (current page/filter; hover-matched highlight via `APPLICATION_CARD_HIGHLIGHT_CLASSES`; right-pointing half-diamond marker to the left via `APPLICATION_CARD_KEYBOARD_MARKER_CLASSES` in a fixed-width slot — `invisible` when not selected so the list does not shift; marker only during keyboard nav, not mouse hover alone; seeds from hovered card while pointer is over it, otherwise starts from the first visible card — no edge wrap); **Enter** opens the detail drawer (`initialFocus` on sheet container, not the header job link); **Escape** closes the drawer and restores keyboard highlight for continued navigation; arrow keys suppress native card hover via `data-scroll-hover-locked` while keyboard highlight is active; mouse leave, click-away, or mouse move clears keyboard highlight and hover seed (drawer close restore excepted)
...
- Admin shell UI (default): `AuthenticatedAppRouter` in `AppPage` picks `ShellAuthenticatedApp` vs classic `AuthenticatedApp` via `applied-dev-ui-mode` (`shell` default, toggle in profile menu or Settings → General); shell lives in `src/components/shell/` — collapsible `AppSidebar` (`applied-dev-sidebar-collapsed`), sticky `AppHeader` with Add Application, `ProfileMenu` (theme, UI toggle, logout), sectioned `SettingsDialog` (General/Data/Archive/Agent/Shortcuts — admin backup/tokens/archive moved here); routes `/` (applications), `/bookmarks` (pinned only), `/archived`; placeholder sidebar items (Analytics, Manage Users, etc.) link to `/`; shell content area flexes full width with padding (no `max-w-3xl`); shared list logic in `useAuthenticatedAppController` + `ApplicationListPage`; classic UI preserved unchanged with centered `max-w-3xl` header
...
- Keyboard shortcut helpers in `src/lib/keyboardShortcut.ts`: `appKeyboardShortcuts()` catalog for the help panel (Global, Admin, Detail Drawer contexts); `isAdminOpenShortcut` (lowercase `a`) and `isAdminBulkArchiveShortcut` (lowercase `e` when admin open — calls bulk archive directly, bypassing confirm; bound only while modal is open) wired in `AdminDialog`; shared guards (`isEditableKeyboardTarget`, `consumeDoubleEscape`, mod-key labels) used across `AuthenticatedApp` and detail sheet; list card keyboard nav in `AuthenticatedApp` via `applicationCardNavigation.ts` (`stepCardIndex`, `resolveNextCardId`, `canHandleApplicationCardNavigation`) and shared hover classes in `applicationCardStyles.ts`
