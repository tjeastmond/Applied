...
- Job URL parse uses `extractJobCompany` and `extractParaformRole`: Y Combinator, Ashby, and Paraform are job boards, not employers; Paraform title/company from JSON-LD, Next data, and og:title patterns; `normalizeJobTitle()` strips `| Y Combinator` and `| Simplify` suffixes on save only (Zod `titleSchema` + repository insert/update rows), not during URL parse or DB reads
